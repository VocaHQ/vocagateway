...
UTF8_ENCODING = "utf-8"
_TOKEN_PADDING = "x" * TEST_TOKEN_PADDING_LENGTH
PADDED_TEST_TOKEN = f"test-{_TOKEN_PADDING}"
...
def test_environment_defaults_to_all_interf_b6e88(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_fresh_install_mints_token_to_xdg_config(
...
def test_honours_xdg_config_and_data_dirs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_reads_token_from_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    home = _isolate_home(monkeypatch, tmp_path)
    token_file = home / CONFIG_DIRECTORY_NAME / APPLICATION_DIRECTORY_NAME / TOKEN_FILE_NAME
...
def test_env_token_overrides_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    home = _isolate_home(monkeypatch, tmp_path)
    token_file = home / CONFIG_DIRECTORY_NAME / APPLICATION_DIRECTORY_NAME / TOKEN_FILE_NAME
    token_file.parent.mkdir(parents=True)
    token_file.write_text("file-token-with-at-least-thirty-two-chars\n", encoding=UTF8_ENCODING)
...
def test_whitespace_only_token_env_is_ignored(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    home = _isolate_home(monkeypatch, tmp_path)
    token_file = home / CONFIG_DIRECTORY_NAME / APPLICATION_DIRECTORY_NAME / TOKEN_FILE_NAME
...
def test_cli_token_from_env_strips_whitespace(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_env_overrides_bind_host_and_port(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(TOKEN_ENVIRONMENT_VARIABLE, PADDED_TEST_TOKEN)
...
def test_custom_token_file_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    home = _isolate_home(monkeypatch, tmp_path)
