...
class _NeverReadyWorker:
    ...
    def __init__(self, *_: object, **_unused: object) -> None:
        ...
        self.context_tokens = MINIMUM_CONTEXT_TOKENS
...
class _WorkerRecorder:
    ...
    def __call__(self, *_: object, **_unused: object) -> _NeverReadyWorker:
...
async def _start_background_load(manager: object) -> None:
    assert await manager.warmup() is False  # type: ignore[union-attr]
    await asyncio.sleep(0)


async def _release_and_reap(manager: object, worker: _NeverReadyWorker) -> None:
    worker.released.set()
    await _settle_load_hold(manager)
    await _wait_for_stop(worker)
...
async def test_a_replacement_load_is_not_started_under_another_models_hold(
    ...
    manager = manager_for(settings, cleanup_enabled=True, cleanup_model=CLEANUP_MODEL_ID)
    _bind_model_files(manager, tmp_path)
    await _start_background_load(manager)
    worker_a = scripted_workers.workers[0]
    ...
    await _release_and_reap(manager, worker_a)
    ...
    await _release_and_reap(manager, worker_b)
    assert manager.host._worker is None
