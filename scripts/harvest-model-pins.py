...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--cleanup",
        action="store_true",
        help="Harvest the transcript-cleanup catalog into app/cleanup_model_pins.json.",
    )
    parser.add_argument("--output", type=Path, default=None)
    args = parser.parse_args(argv)
    # The cleanup artifacts are a separate catalog with a separate pin file:
    # a text model is not a speech engine and must never share their namespace.
    catalog = (
    ...
    if args.output is None:
        args.output = CLEANUP_PINS_PATH if args.cleanup else PINS_PATH
    ...
    live_ids = {model.id for model in catalog if not model.retired}
    ...
    script_name = "scripts/harvest-model-pins.py"
    invocation = f"{script_name} --cleanup" if args.cleanup else script_name
    extra = (
        " A transcript-cleanup model with no entry here cannot be installed at all:"
        " unverifiable weights must never reach a runtime launch."
        if args.cleanup
        else ""
    )
    payload = {
        "_comment": (
            f"Generated by {invocation}. Each entry pins the "
            ...
            f"digest means the upstream bytes changed.{extra}"
        ),
