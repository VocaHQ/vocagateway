...
def exposure_network_panel(*, is_mac: bool = False) -> str:
    ...
    return f"""
      <div class="exposure-panel" role="note">
        ...
        <p>Any device that can reach this host can open the gateway port. Transcription and admin still need the bearer token.</p>
        ...
        <p class="muted small">Bind to a single address with <code>VOCAPHONE_BIND_HOST</code> if you want to stop listening on every interface.</p>
      </div>
    """
