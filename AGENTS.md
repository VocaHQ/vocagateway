...
| `compose.prod.yaml` | The same deployment pulling a published image, once a release has pushed one. Self-contained, so it can be downloaded on its own; `tests/test_compose_files.py` keeps it from drifting |
